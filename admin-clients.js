(() => {
  const mod = window.EDMAdminClients = {
    async load() {
      ...
      const { data, error } = await app.db.from('profiles').select('id,first_name,last_name,phone,role,external_client_id,created_at').order('created_at', { ascending: false });
      ...
      app.$('docClient').innerHTML = '<option value="">Choisir</option>' + app.clients.map((c) => `<option value="${c.id}">${app.esc(`${c.first_name || ''} ${c.last_name || ''}`.trim() || c.id)}</option>`).join('');
    },
    render(rows) {
      const app = window.EDMAdmin;
      app.$('clientResults').innerHTML = rows.length ? `<div class="tablewrap"><table class="table"><thead><tr><th>Client</th><th>Téléphone</th><th>Identifiant</th><th></th></tr></thead><tbody>${rows.map((c) => `<tr><td>${app.esc(`${c.first_name || ''} ${c.last_name || ''}`.trim() || 'Sans nom')}</td><td>${app.esc(c.phone || '-')}</td><td>${app.esc(c.external_client_id || c.id)}</td><td><button class="btn ghost" data-client="${c.id}">Ouvrir</button></td></tr>`).join('')}</tbody></table></div>` : '<p class="muted">Aucun résultat.</p>';
    ...
    async search() {
      ...
      const { data: vehicles } = await app.db.from('vehicles').select('user_id,plate,brand,model').or(`plate.ilike.%${term}%,brand.ilike.%${term}%,model.ilike.%${term}%`);
      const ids = new Set((vehicles || []).map((v) => v.user_id));
      this.render(app.clients.filter((c) => [c.first_name,c.last_name,c.phone,c.external_client_id,c.id].some((v) => String(v || '').toLowerCase().includes(term)) || ids.has(c.id)));
    },
    async show(id) {
      const app = window.EDMAdmin;
