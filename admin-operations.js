(() => {
  ...
  async function prepare(q, root) {
    ...
    if (!orderNumber) throw new Error('Numéro d’ordre obligatoire.');
    ...
    if (!appointmentId) {
      ...
      const created = await A().db.from('appointments').insert({
        ...
        ends_at: end,
        status: 'confirmed',
        notes: `Intervention liée au devis ${q.quote_number || q.id}`,
    ...
    if (!existing.data?.length) {
      ...
      const authorized = Array.isArray(request.data?.services) ? request.data.services : [];
  ...
  function bind() {
    document.querySelector('[data-page="operations"]')?.addEventListener('click', () => load().catch((e) => A().status('operationStatus', e.message || 'Atelier indisponible.', true)));
    document.getElementById('operationRefresh')?.addEventListener('click', () => load().catch((e) => A().status('operationStatus', e.message || 'Actualisation impossible.', true)));
  }
