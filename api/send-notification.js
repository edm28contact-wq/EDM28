...
export default async function handler(req, res) {
  ...
  try {
    ...
    const values = { ...(req.body?.values || {}), client_name: clean(`${profile.first_name || ''} ${profile.last_name || ''}`.trim(), 120) || 'Client', business_name: business.business_name || 'EDM28', booking_url: business.booking_url || '' };
    ...
    if (logId) await rest(`outbound_notifications?id=eq.${encodeURIComponent(logId)}`, authorization, {
      method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: 'sent', provider_message_id: result.id || null, sent_at: new Date().toISOString(), error_message: null })
    });
    ...
    try {
      if (logId && req.headers.authorization) await rest(`outbound_notifications?id=eq.${encodeURIComponent(logId)}`, req.headers.authorization, {
        method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: 'failed', error_message: clean(error.message, 1000) })
      });
    } catch (_) {}
