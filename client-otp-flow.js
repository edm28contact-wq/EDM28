(() => {
  ...
  async function sendCode() {
    ...
    try {
      ...
      $('otpCode').focus();
      setMessage(`Code envoyé à ${client.email}. Saisissez le code reçu.`);
  ...
  async function verifyCode() {
    ...
    if (!email || token.length < 6 || token.length > 10) return setMessage('Entrez le code reçu par email.', true);
  ...
  async function install() {
    ...
    row.insertAdjacentHTML('afterend', `
      <div id="otpPanel" class="card hidden" style="margin-top:14px">
        ...
        <label>Code reçu
          <input id="otpCode" inputmode="numeric" autocomplete="one-time-code" maxlength="10" placeholder="12345678">
        </label>
