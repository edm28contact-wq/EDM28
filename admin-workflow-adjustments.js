(() => {
  ...
  const modeLabels = {
    ...
    edm_disbursement: 'Débours EDM28'
  };
  ...
  function decorateQuoteCard(card) {
    ...
    if (mode === 'client_direct') {
      note.innerHTML = '<strong>Achat direct par le client</strong>EDM28 doit renseigner la référence de chaque pièce dans le devis. Le client achète lui-même les références indiquées. Si le client achète une autre référence, la responsabilité d’EDM28 ne peut pas être engagée pour cette erreur d’achat.';
    } else if (mode === 'edm_disbursement') {
      note.innerHTML = '<strong>Débours EDM28</strong>EDM28 achète au nom et pour le compte du client dans la limite autorisée. Le client rembourse exactement le justificatif fournisseur, sans marge ni commission.';
    } else {
      note.innerHTML = '<strong>Mode d’achat des pièces non renseigné</strong>Cette ancienne demande ne contient pas encore le nouveau choix achat direct / débours.';
    }
  ...
  async function hidePublishedRepairOrders() {
    ...
    try {
      ...
      cards.forEach((card) => {
        const hide = published.has(card.dataset.repairOrder);
        card.dataset.publishedHidden = hide ? '1' : '0';
      });
  ...
  function normalizeOrderPageCopy() {
    ...
    if (paragraph) paragraph.textContent = 'Préparer les OR non publiés. Dès qu’un OR est validé et publié au client, il disparaît automatiquement de cette file.';
  }
